...
class Model(nn.Module):
    def __init__(self, embed, config, device):
        ...
        self.emoji_bias = nn.Parameter(torch.zeros(config.batch_size, ))
        self.word_bias = nn.Parameter(torch.zeros(config.max_seq_length, ))
        self.relu = nn.ReLU()
        self.LSTM_encoder = nn.LSTM(config.word_embedding_size, config.lstm_hidden_size,
    ...
    def sentence_attention(self, e, x):
        """
        attend on emojis
        :param e: tensor
                shape: (batch, emoji_embed_size * 2)
                emoji embeds
        :param x: tensor
        ...
        """
        # shape: (batch, batch)
        u = self.relu(torch.mm(e, x.permute(1, 0)) + self.emoji_bias)
        alpha = F.softmax(u)
        # shape: (batch, emoji_embed_size * 2)
        vt = torch.matmul(alpha.permute(1, 0), e)

        return vt

    def word_attention(self, e, x):
        """
        attend on words
        :param e: tensor
                ...
                emoji embeds
        :param x: tensor
                shape: (batch, max_seq_length, word_embedding_size)
                sequence embeds
        :return vt: tensor
                shape: (batch, max_seq_length, emoji_embed_size * 2)
        """
        # shape: (batch, 1, max_seq_length)
        u = self.relu(torch.matmul(e, x.permute(0, 2, 1)) + self.word_bias)
        alpha = F.softmax(u)
        # shape: (batch, max_seq_length, emoji_embed_size * 2)
    ...
    def forward(self, input_seqs, input_emojis):
        ...
        encoder_output, _ = self.LSTM_encoder(seq_embed)
        ...
        # shape: (batch_size, emoji_embed_size * 2)
        emoji_embed = torch.cat((pos_embeds, neg_embeds), 1)

        # shape: (batch_size, emoji_embed_size * 2)
        vt = self.sentence_attention(emoji_embed, encoder_output)

        # shape: (batch_size, 1, emoji_embed_size * 2)
        vt = vt.unsqueeze(1)

        # shape: (batch_size, max_seq_length, emoji_embed_size * 2)
        vt = self.word_attention(vt, seq_embed)
