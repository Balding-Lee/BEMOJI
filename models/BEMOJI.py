...
class FineTuneConfig:
    def __init__(self):
        ...
        self.pre_train_chinese_texts_max_length = 64
        self.pre_train_english_texts_max_length = 96
        self.pre_train_chinese_emoji_max_length = 32
        self.pre_train_english_emoji_max_length = 90
    ...
    def pre_train_config(self, batch_size, epochs, learning_rate, weight_decay,
                         from_path, device):
...
class BEMOJI(nn.Module):
    ...
    def forward(self, input_seqs, input_emoji_defs, input_prompt):
        ...
        :param input_prompt: list
                带有 prompt 的序列
        """
        if self.args.mode == 'pre_train_chinese':
            context_tokens = self.tokenizer.batch_encode_plus(input_seqs,
                                                              add_special_tokens=True,
                                                              max_length=self.config.pre_train_chinese_texts_max_length,
                                                              padding='max_length',
                                                              truncation='longest_first',
                                                              return_tensors='pt')
            emoji_tokens = self.tokenizer.batch_encode_plus(input_emoji_defs,
                                                            add_special_tokens=True,
                                                            max_length=self.config.pre_train_chinese_emoji_max_length,
                                                            padding='max_length',
                                                            truncation='longest_first',
                                                            return_tensors='pt')
            prompt_tokens = self.tokenizer.batch_encode_plus(input_prompt,
                                                             ...
                                                             truncation='longest_first',
                                                             return_tensors='pt')
        elif self.args.mode == 'pre_train_english':
            context_tokens = self.tokenizer.batch_encode_plus(input_seqs,
                                                              add_special_tokens=True,
                                                              max_length=self.config.pre_train_english_texts_max_length,
                                                              padding='max_length',
                                                              truncation='longest_first',
                                                              return_tensors='pt')
            emoji_tokens = self.tokenizer.batch_encode_plus(input_emoji_defs,
                                                            add_special_tokens=True,
                                                            max_length=self.config.pre_train_english_emoji_max_length,
                                                            padding='max_length',
                                                            truncation='longest_first',
                                                            return_tensors='pt')
            prompt_tokens = self.tokenizer.batch_encode_plus(input_prompt,
