...
from models import BEMOJI_non_emoji
...
def train(args, device):
    config = BEMOJI_non_emoji.Config(
        args,
    ...
    save_path = os.path.join(fp.model_parameters, 'BEMOJI_{}_parameters.bin'.format(args.dataset))
    datasets = BEMOJI_non_emoji.PreprocessDataset()
    ...
    model = BEMOJI_non_emoji.BEMOJICLS(args, config, device).to(device)
    ...
    if args.dataset != 'SST-2' and args.dataset != 'ECISA':
        ...
        with open(os.path.join(fp.chinese_sa_dataset, 'SMP2019-ECISA/result.txt'), 'a', encoding='utf-8') as f:
